...
Phase 6 — Pharmacogenomic Contexts and Explainable Modeling (XAI) is in progress. Notebooks `600 — Program–Drug Associations` and `601 — Explainable Predictive Modeling` are complete under frozen specifications, with their stable handoffs registered in `config/artifact_registry.json`. Notebook 601 evaluated all 281 frozen eligible GDSC drugs under repeated lineage-stratified out-of-fold evaluation; 125 satisfy the prospectively frozen internal predictive-validity gate for subsequent primary program-level SHAP interpretation. This establishes model-attribution eligibility only and does not imply biological importance, mechanism, external reproducibility, clinical predictiveness, or therapeutic relevance. Notebook 602 attribution was prospectively frozen on 2026-09-21 and remains pending execution; no SHAP attribution result has been inspected. Notebook 603 cross-screen replication remains pending prospective design/freeze, and no cross-screen replication result has yet been inspected.
