...
| [`contracts/`](contracts/) | Prospective analysis contracts organized by phase, with lifecycle rules in its README. |
...
Roadmap v3.2 adds **Phase 4B — Secondary Molecular Context Characterization** without reopening Phase 4. Notebook `108 — TCGA Somatic Mutation Acquisition and Audit` is complete / frozen and provides the audited exact-sample mutation-resource handoff. Notebooks `450 — Secondary Genomic Context Characterization` and `451 — Locus-Level Methylation–Expression Characterization` are complete / frozen; notebook 451's seven registered `phase4b.451.*` handoffs are under `data/processed/secondary_characterization/`. Positive, negative, lineage-specific, heterogeneous, or non-recurrent outcomes are all valid, and Phase 4B results cannot redefine frozen programs. Methylation-expression associations are contextual computational evidence, not causal epigenetic regulation, and TCGA tumors are not resistance/sensitivity labeled.

The historical input contract for notebook 401 remains available at [`contracts/phase4/PHASE4_401_INPUT_CONTRACT.md`](contracts/phase4/PHASE4_401_INPUT_CONTRACT.md). It preserves the provenance and interpretation rules that governed consensus construction, including the distinction between unique RNA correspondence events, tumor structural families, and tumor cross-omic arms.

The notebook-450 analysis contract is available at [`contracts/phase4b/PHASE4B_450_ANALYSIS_CONTRACT.md`](contracts/phase4b/PHASE4B_450_ANALYSIS_CONTRACT.md). The notebook-451 contract and its execution/freeze addendum are at [`contracts/phase4b/PHASE4B_451_ANALYSIS_CONTRACT.md`](contracts/phase4b/PHASE4B_451_ANALYSIS_CONTRACT.md). These preserve their historical prospective methods and execution provenance without converting recurrence or methylation-expression association into causal evidence.

Phase 1 notebooks `107 — DepMap RNAi Acquisition and Audit` and `108 — TCGA Somatic Mutation Acquisition and Audit` are complete. Their frozen handoffs provide distinct upstream resources for Phase 5 RNAi characterization and Phase 4B genomic-context characterization, respectively. Mutation-resource availability remains lineage dependent and is not treated as missing completely at random.

Phase 5 — Functional Vulnerabilities is closed and frozen after completion of notebooks `500 — CRISPR Associations`, `501 — RNAi Associations`, and `502 — Integrated Vulnerability Mapping`. CRISPR and RNAi remain separate functional-genomics evidence dimensions with their own frozen eligibility criteria and effect scales. Notebook 502 preserves platform-specific coefficients, FDR values, coverage, and lineage-aware context without statistical pooling, a common post-hoc coverage threshold, joint cross-platform FDR, or a composite vulnerability ranking. Cross-platform concordance is interpreted as complementary computational evidence rather than independent validation because biological context and cell-line populations overlap. The derived Phase 5 artifacts are registered with frozen identity and lineage in `config/artifact_registry.json`.

Phase 6 — Pharmacogenomic Contexts and Explainable Modeling (XAI) is complete under frozen specifications. Notebooks `600 — Program–Drug Associations`, `601 — Explainable Predictive Modeling`, `602 — SHAP Attribution and Stability Analysis`, and `603 — Cross-Screen Replication` are implemented and executed, with their stable handoffs registered in `config/artifact_registry.json`. Notebook 601 evaluated all 281 frozen eligible GDSC drugs under repeated lineage-stratified out-of-fold evaluation; 125 satisfied the prospectively frozen internal predictive-validity gate and entered notebook 602. Notebook 602 used persisted held-out fitted state without refitting, produced exact program-level attribution with numerical reconstruction of the frozen predictions, characterized repeat and lineage-composition sensitivity, and added frozen Phase 4/4B biological context without creating gene-level SHAP. These remain internal GDSC model-attribution results. Notebook 603 cross-screen replication is complete under the prospectively frozen 2026-09-22 specification. Of 177 primary evaluable external hypotheses, 33/79 CTRP and 4/98 PRISM hypotheses met the frozen screen-level replication rule. None of the 40 hypotheses evaluable in both external screens replicated in both, and 4 CTRP replications received prespecified non-overlapping-model corroboration. These results remain computational cross-screen evidence rather than independent biological or clinical validation.
...
- [`decisions/004_secondary_molecular_context_characterization.md`](decisions/004_secondary_molecular_context_characterization.md) — formalizes roadmap v3.2, notebooks 108 and 450–451, the post-freeze characterization boundary, and optional publication of secondary analyses.
...
- [`../config/raw_data_registry.json`](../config/raw_data_registry.json) is the source for dataset releases, source files, and raw-data provenance used by the implemented workflow, including the TCGA/GDC somatic-mutation resource audited in notebook 108.
- [`../config/artifact_registry.json`](../config/artifact_registry.json) records frozen derived-artifact identity and lineage, including completed Phase 4B, Phase 5, notebook-600, and notebook-601 downstream-required handoffs.
