...
GDSC, CTRP, and PRISM characterize resistance-like pharmacogenomic contexts and provide the outcomes for prespecified explainable predictive modeling. Phase 6 contains the explicit XAI component of the project, including SHAP feature attribution, attribution stability, and biologically contextualized interpretation after model validity and evaluation design are established.
...
Frozen evidence from Phase 4, Phase 4B, and Phases 5–8 is integrated into a transparent program–vulnerability–compound framework. Evidence dimensions remain separately traceable; convergence supports computational prioritization only. The final catalog may provide a structured computational-to-experimental handoff without prescribing experimental outcomes or protocols.
...
* sensitivity of feature ranking to model class;
* agreement or disagreement between SHAP and simpler model coefficients or effect estimates; and
* hierarchical biological contextualization of stable model attribution using only frozen upstream biological annotations and representations.

## 8.3 Hierarchical Biological Contextualization

XAI outputs must distinguish **model attribution** from **biological contextualization**.

Attribution can be claimed only for features actually supplied to the fitted model. Therefore:

* if a model uses consensus-program scores, SHAP attribution is program-level;
* if a model uses gene-level features, gene-level SHAP attribution may be reported only under the same leakage-safe training and evaluation structure;
* gene-level, pathway-level, epigenetic-regulator, methylation, or Phase 4B context mapped downstream from a program-level attribution is contextual annotation, not additional SHAP evidence.

Where relevant, a stable attributed program may be contextualized through the following frozen hierarchy:
...
6. Phase 4B locus-level methylation-expression or genomic context where informative.

This hierarchy is intended to connect model behavior back to the biological framework without creating pseudo-resolution. A program-level SHAP result must not be redistributed mathematically across constituent genes and then presented as gene-level SHAP unless those genes were explicit model features.
...
SHAP must never be described as identifying a master regulator or causal driver. It can support prioritization, model interpretation, biological contextualization, and hypothesis generation only.

---

# 9. Integrated Evidence and Computational-to-Experimental Handoff Policy
...
## 9.1 Final Candidate Handoff

Notebook 904 should generate a structured, frozen computational evidence handoff for retained candidate relationships. Where information exists, each candidate record should preserve:
...
* pharmacogenomic association and replication evidence;
...
* lineage breadth, heterogeneity, and relevant coverage limitations;
* external or orthogonal support;
* explicit conflicting or missing evidence; and
* a concise computational rationale for possible downstream experimental evaluation.

Because the broader approved project includes experimental work in sarcoma models, RMS or OS relevance may be annotated when it is directly supported by the computational datasets. Lack of sarcoma/RMS/OS coverage or evidence must be recorded as unavailable or unsupported rather than inferred from other lineages.

The handoff is an evidence package, not an experimental result. It must not prescribe assay design, concentration, dose, schedule, therapeutic efficacy, or clinical actionability.

---

# 10. Interpretability and Claim Boundaries

Permitted language includes recurrent program, candidate program, resistance-like context, secondary genomic context, locus-level methylation-expression association, putative vulnerability, computational association, explainable model attribution, biological contextualization, perturbational hypothesis, integrated evidence, computational-to-experimental handoff, internal robustness, cross-system reproducibility, and cross-dataset replication.
...
Integrated program–vulnerability–compound evidence may support computational prioritization but must not be labeled as validated therapeutic targeting. A computational-to-experimental handoff supports downstream study design; it is not experimental validation itself.
