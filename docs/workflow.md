...
**Status:** **COMPLETE UNDER FROZEN SPECIFICATIONS — notebooks 600–603 implemented and executed.**
...
- **Primary inputs:** frozen Phase 4 consensus programs and the audited GDSC, CTRP, and PRISM pharmacogenomic interfaces. GDSC is developmental/internal; CTRP and PRISM are reserved as external cross-screen resources.
- **Notebook 600 completion:** 281 GDSC drugs satisfied the frozen `20 / 3 / 100` lineage-support rule. The complete developmental/internal primary family contained 843 lineage-adjusted program–drug tests with joint Benjamini–Hochberg FDR control. Stable notebook-600 score, mapping, eligibility, resource-specific analysis-universe, association, and metadata handoffs are registered in `config/artifact_registry.json`.
- **Notebook 601 completion:** all 281 eligible GDSC drugs were evaluated independently with the frozen lineage-only baseline and fixed lineage-plus-three-program linear model under `5-fold lineage-stratified cross-validation × 5 repeats`, seed `601`, using identical held-out partitions for both models. The primary criterion was incremental out-of-fold `R²` beyond lineage. No feature selection, hyperparameter tuning, model-family search, post-performance threshold change, or drug rescue was used.
- **Predictive-validity handoff:** 125/281 drugs satisfy all three prospectively frozen internal predictive-validity criteria for subsequent primary program-level SHAP interpretation. Passing this gate establishes attribution eligibility only; failure remains a valid result and passing does not establish biological importance, mechanism, external reproducibility, clinical predictiveness, or therapeutic relevance.
- **Secondary unseen-lineage stress test:** notebook 601 also completed 2,961 drug × held-out-lineage evaluations. Relative performance versus the training-derived intercept-only comparator was often improved, while absolute unseen-lineage prediction remained weak overall. This analysis is descriptive only and cannot rescue or exclude drugs from the primary gate.
- **Repository handoff:** notebook-601 partitions, out-of-fold predictions, fold-specific primary-model parameters, fold-level lineage effects, repeat-level performance, LOLO performance, integrated drug-level results, and analysis metadata are registered as frozen `phase6.601.*` artifacts under `data/processed/pharmacogenomic_contexts`. Notebook 602 consumes those fitted-state handoffs without refitting; its stable held-out attribution, repeat-, lineage-, drug-program-, program-block-, biological-context, and analysis-metadata interfaces are registered as frozen `phase6.602.*` artifacts under `data/processed/pharmacogenomic_contexts`.
- **Notebook 602 boundary:** completed under the attribution specification prospectively frozen on 2026-09-21. Primary attribution remained restricted to the 125 notebook-601 `shap_eligible` drugs and consumed persisted notebook-601 OOF predictions and fitted-state handoffs without refitting. Exact interventional linear SHAP reconstructed persisted predictions to numerical precision; repeat-level magnitude, coefficient-direction stability, pooled-versus-lineage-balanced summaries, program-block versus lineage attribution, and frozen biological contextualization were retained as descriptive model-behavior evidence. Attribution remains program-level; downstream gene/pathway mapping is biological contextualization rather than gene-level SHAP evidence.
- **Notebook 603 completion:** the cross-screen replication specification was prospectively frozen on 2026-09-22 before CTRP/PRISM replication-result inspection. Primary replication remained restricted to frozen notebook-600 GDSC FDR-controlled hypotheses with frozen exact-compound matching and external eligibility. Of 177 primary evaluable external hypotheses, 33/79 CTRP and 4/98 PRISM hypotheses met the frozen screen-level rule. Four CTRP replications also received prespecified non-overlapping-model corroboration; none of the 40 hypotheses evaluable in both screens replicated in both. Notebook-601 and notebook-602 evidence was appended only after replication statuses were fixed and did not define, rescue, or downgrade replication.
- **XAI boundary:** SHAP and related feature-attribution methods explain model behavior. They do not establish causal mechanisms, validated biomarkers, therapeutic targets, clinical prediction, or therapeutic efficacy. Attribution remains interpretable only within the reported predictive-validity and evaluation boundaries.
