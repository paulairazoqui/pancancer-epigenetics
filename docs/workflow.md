...
**Status:** **IN PROGRESS — notebooks 600–601 complete; notebook 602 specification frozen / execution pending; notebook 603 pending prospective design/freeze.**
...
- **Repository handoff:** notebook-601 partitions, out-of-fold predictions, fold-specific primary-model parameters, fold-level lineage effects, repeat-level performance, LOLO performance, integrated drug-level results, and analysis metadata are registered as frozen `phase6.601.*` artifacts under `data/processed/pharmacogenomic_contexts`. The fitted-state handoffs preserve the already executed primary models for notebook-602 attribution without downstream refitting.
- **Notebook 602 boundary:** the attribution specification was prospectively frozen on 2026-09-21 and has not yet been executed. Primary attribution is restricted to the 125 notebook-601 `shap_eligible` drugs and consumes persisted notebook-601 OOF predictions and fitted-state handoffs without refitting. Exact interventional linear SHAP, repeat-level `mean(abs(SHAP))` aggregation, continuous magnitude/coefficient stability characterization, and pooled-versus-lineage-balanced lineage summaries are frozen. Attribution remains program-level; downstream gene/pathway mapping is biological contextualization rather than gene-level SHAP evidence.
