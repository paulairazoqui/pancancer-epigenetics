...
- **Repository handoff:** notebook-601 partitions, out-of-fold predictions, fold-specific primary-model parameters, fold-level lineage effects, repeat-level performance, LOLO performance, integrated drug-level results, and analysis metadata are registered as frozen `phase6.601.*` artifacts under `data/processed/pharmacogenomic_contexts`. Notebook 602 consumes those fitted-state handoffs without refitting and persists stable held-out attribution, repeat-, lineage-, drug-program-, program-block-, biological-context, and analysis-metadata interfaces under `data/processed/pharmacogenomic_contexts`.
